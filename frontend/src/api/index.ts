...
export function subscribeToEvents(onEvent: (e: AppEvent) => void): () => void {
    ...
    // Native EventSource header support is limited/non-existent in browser.
    // But our server accepts token in query param:
    ...
    es.onerror = (e) => {
        // SSE automatic reconnection will happen, but we log it
